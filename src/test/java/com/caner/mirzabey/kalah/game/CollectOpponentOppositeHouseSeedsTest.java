...
public class CollectOpponentOppositeHouseSeedsTest {
    ...
    public void init() {
        ...
        playerSide.setPit(prepareHouses(ON_MOVE_HOUSE_SEED_COUNT, MOVE - 1));

        Side opponentSide = game.getBoard().getOpponentSideByUsername(PLAYER_USERNAME);
        opponentSide.setTurnOnMe(false);
        opponentSide.setPit(prepareHouses(OPPONENTS_OPPOSITE_HOUSE_SEED_COUNT, OPPONENTS_OPPOSITE_HOUSE_INDEX));
    ...
    public void test() throws Exception {
        ...
        int  playerStoreSize = result.getBoard().getPlayerSideByUsername(PLAYER_USERNAME).getKalah().size();
        int opponentsOppositeHouseSize = result.getBoard().getOpponentSideByUsername(PLAYER_USERNAME).getPit().get(
